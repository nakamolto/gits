...
class SqljsStatementAdapter implements SqliteStatementLike {
  constructor(private readonly stmt: Statement) {}

  run(params?: unknown[] | Record<string, unknown>): unknown {
    if (Array.isArray(params)) this.stmt.run(params as any);
    else if (params && typeof params === 'object') this.stmt.run(params as any);
    else this.stmt.run();
    return undefined;
  }

  all(params?: unknown[] | Record<string, unknown>): unknown[] {
    if (Array.isArray(params)) this.stmt.bind(params as any);
    else if (params && typeof params === 'object') this.stmt.bind(params as any);
...
class SqljsDatabaseAdapter implements SqliteDatabaseLike {
  ...
  prepare(sql: string): SqliteStatementLike {
    return new SqljsStatementAdapter(this.db.prepare(sql));
  }
}
